...
#
# This rewritten version makes two primary overlay plots:
...
#   figs/s1b/p_EI_original_with_MSN_all_regions_with_jorstad_cortex_overlay.{pdf,jpg}
...
original_wide <- calculate_region_props(
  ...
  mutate(
    ...
    log2_EI_ratio_no_MSN = log2(EI_ratio_no_MSN),
...
can_add_fit_label <- function(df) {
  nrow(df) >= 4 &&
    n_distinct(df$log2_EI_ratio[is.finite(df$log2_EI_ratio)]) >= 2 &&
...
make_overlay_plot <- function(broad_df,
  ...
  p <- ggplot() +
    ...
    labs(
      ...
      caption = paste(
        broad_caption,
        "Blue points/line: cerebral cortical regions only. E:I uses only cortical excitatory and inhibitory supercluster families crosswalked to Jorstad et al. (2023) neocortical E/I subclasses.",
        "Blue excludes MSN and excludes thalamic, hippocampal, cerebellar, and midbrain cell types. Lines are OLS fits with 95% CI.",
...
p_overlay_no_msn <- make_overlay_plot(
  ...
  broad_caption = paste(
    ...
    "Medium spiny neurons are excluded from the inhibitory denominator in this plot."
  ),
  output_stem = "figs/s1b/p_EI_original_all_regions_with_jorstad_cortex_overlay"
)
...
p_overlay_with_msn <- make_overlay_plot(
  ...
  plot_subtitle = "Grey denominator now includes inhibitory interneuron plus MSN projection classes; blue Jorstad points are unchanged.",
  broad_caption = paste(
    "Grey points/line: all mapped brain regions. E:I = excitatory-projection / (inhibitory-interneuron + inhibitory-projection/MSN) superclusters.",
    "This is the broader all-anatomy inhibitory denominator including medium spiny neurons."
  ),
  output_stem = "figs/s1b/p_EI_original_with_MSN_all_regions_with_jorstad_cortex_overlay"
)
...
cat("  figs/s1b/p_EI_original_with_MSN_all_regions_with_jorstad_cortex_overlay.{pdf,jpg}\n")
