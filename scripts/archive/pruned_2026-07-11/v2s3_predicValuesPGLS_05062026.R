# ============================================================
# Study 3 — PIPELINE v2 ("replace neocortex grey")
# Predicted vs observed brain-region volumes in Homo sapiens
# Phylogenetic GLS (part-whole corrected) + relationship to rCMRGlc
#
# Same engine as the tidy s3_predicValuesPGLS_04062026.R (pipeline 1), but this
# is the analysis variant where Neocortex grey is REPLACED by the de-overlapped
# remainder, and the rCMRGlc correspondence is taken from the v2 upstream table.
#   * reads  data_intermediate/Heiss_Stephan_data_v2.csv
#            (built by s3v2_0_Heiss_Stephan_and_table1_05062026.R)
#   * writes figs/checks/tables under .../s3v2/  (does NOT touch pipeline-1 output)
#
# Phase 1.2 — neocortex de-overlap is ON here (that is the whole point of v2):
#   Neocortex grey (remaining) = NeoG_Frahm - V1(ASG_Sousa) - Insula(grey)
#   DEFAULT below honours your full definition (subtract V1 AND insula). NOTE the
#   N cost: V1 is present for all 24 neocortex species but insula is missing for
#   15 of them, so the full definition drops neocortex to n=9. Insula is only
#   ~0.5-1.1% of neocortex grey, so two escape hatches preserve power:
#     - neocortex_subtract_insula = FALSE  -> V1-only remainder, n=24 (~1% insula left in)
#     - impute the tiny insula term (see s3_1_phylo_multiple_imputation) -> n=24, full def.
#
# Phase 3.2 (deferred): the v2 upstream currently passes neocortex rCMRGlc
# through unchanged (stub). When reweighted, v2 results update automatically.
# ============================================================
...
CONFIG <- list(
  ...
  # Phase 1.2 — neocortex de-overlap. ON for v2 (this is the "replace" pipeline).
  deoverlap_neocortex      = TRUE,   # TRUE => NeoG - V1 (- insula); FALSE => raw NeoG_Frahm
  ...
  # I/O — v2 paths so pipeline-1 outputs are never overwritten
  dir_figs   = "figs/s3v2/all",
  dir_checks = "checks/s3v2/all",
  dir_tables = "tables/s3v2/all"
)
...
# LOAD
# ============================================================
heiss_stephan_tbl <- read.csv("data_intermediate/Heiss_Stephan_data_v2.csv")
