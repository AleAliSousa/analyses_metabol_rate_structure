...
CONFIG <- list(
  ...
  # I/O
  dir_figs   = "figs/s4_structured/all",
  dir_checks = "checks/s4_structured/all",
  dir_tables = "tables/s4_structured/all"
)
...
# Align tree tip labels with structured dataset nomenclature
tr$tip.label[tr$tip.label == "Gorilla_gorilla"] <- "Gorilla_sp."
tr$tip.label[tr$tip.label == "Pongo_pygmaeus"] <- "Pongo_sp."
tr$tip.label[tr$tip.label == "Lagothrix_lagotricha"] <- "Lagothrix_lagothricha"
tr$tip.label[tr$tip.label == "Tarsius_syrichta"] <- "Tarsius_sp."
tr <- ape::drop.tip(tr, "Pongo_abelii") # Drop extra Pongo tip to avoid duplicates
Stephan_primates <- read.csv("/Users/crossmodal/Library/CloudStorage/OneDrive-AllenInstitute/Species/Evo-M1-Trait-Data/__merging_volumes/volumes_wide_select.csv")
...
# Force underscores in Species to match tree format
