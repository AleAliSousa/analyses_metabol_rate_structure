...
pdf("figs/s3/all/phylo_gls_region_plots.pdf", width = 7, height = 5)
...
dir.create("figs/s3/all/phylo_gls_region_jpgs", showWarnings = FALSE)
...
iwalk(
  ...
  function(p, region_label) {
    ...
    ggsave(
      filename = file.path(
        "figs/s3/all/phylo_gls_region_jpgs",
...
# ============================================================
# PLOT VII + TABLE: 12-way quadratic comparison
#   4 structure configurations x 3 PGLS evolution models = 12 cells
#
#   For each cell we fit:  Diff.pre ~ poly(rCMRGlc, 2, raw = TRUE)
#   on the *included* subset of the 13 brain structures.
#
#   Goal: test whether the quadratic shape / sign of curvature /
#   significance from p_quad_ci survives perturbations of
#     (a) which white-matter structure(s) are excluded, and
#     (b) which phylogenetic evolution model is used.
#
#   Implementation note on mu_source:
#     The published quad_fit / p_quad_ci is built from `core_df`,
#     which was produced by run_models_for_structure(..., mu_source = "trait")
#     using specs_plot12. To guarantee the (Exclude Neocortex white,
#     Brownian) cell here reproduces the published quad_fit
#     coefficients, we re-use mu_source = "trait" below. Switching to
#     mu_source = "predictor" would silently change Diff.pre and
#     defeat the verification target.
# ============================================================

# --- Evolution-model grid (mirrors specs_plot3 ordering) ---
specs_compare <- list(
  list(
    name   = "Independence (λ=0)",
    type   = "Fixed",
...
# --- Structure-config grid (exclusion lists, in pretty label form) ---
...
# --- Run PGLS per (evolution model x structure), compute Diff.pre ---
# Note: PGLS fits are per-structure, so the structure-exclusion filter
# only affects the downstream lm(), not the PGLS Predicted values.
diff_pre_long <- purrr::map_dfr(specs_compare, function(spec) {
  per_struct <- purrr::map_dfr(target_cols, function(vn) {
    pp <- prep_structure(vn, data = data, tr = tr, human = "Homo_sapiens")
    run_models_for_structure(pp, list(spec), want_ci = FALSE, mu_source = "trait")
  })
...
# --- Fit one quadratic per cell -> 12-row stats table ---
quad_grid <- purrr::map_dfr(structure_levels, function(sc_name) {
  excl <- structure_configs[[sc_name]]
  purrr::map_dfr(evolution_levels, function(ev_name) {
    sub <- diff_pre_with_rCMRGlc %>%
      filter(evolution_model == ev_name) %>%
      mutate(excluded = Structure %in% excl)
    fit_df <- sub %>% filter(!excluded)
    n_used <- nrow(fit_df)
    ...
    if (n_used < 4) {
      return(tibble(
        ...
        adj_R2      = NA_real_, AIC         = NA_real_, BIC         = NA_real_,
    ...
    summ <- summary(fit)
    coefs <- coef(summ)  # rows: (Intercept), poly...1, poly...2
    ...
    tibble(
      ...
      n_used           = n_used,
      n_excluded       = n_excl,
      intercept        = unname(coefs[1, "Estimate"]),
      b_linear         = unname(coefs[2, "Estimate"]),
      b_quadratic      = unname(coefs[3, "Estimate"]),
      p_intercept      = unname(coefs[1, "Pr(>|t|)"]),
      p_linear         = unname(coefs[2, "Pr(>|t|)"]),
...
# --- Build prediction grid (curve + 95% CI) per cell ---
xg_compare <- seq(10, 40, length.out = 300)

quad_pred_grid <- purrr::map_dfr(structure_levels, function(sc_name) {
  excl <- structure_configs[[sc_name]]
  purrr::map_dfr(evolution_levels, function(ev_name) {
    sub <- diff_pre_with_rCMRGlc %>%
      filter(evolution_model == ev_name) %>%
...
# --- Build per-cell point data (included vs excluded) ---
points_grid <- purrr::map_dfr(structure_levels, function(sc_name) {
  excl <- structure_configs[[sc_name]]
  diff_pre_with_rCMRGlc %>%
    mutate(
      structure_config = sc_name,
      excluded         = Structure %in% excl
    )
})

# --- Lock facet ordering (rows = structure_config, cols = evolution_model) ---
...
p_quad_compare_12way <- ggplot() +
  ...
  # Included points: black circles
  geom_point(
    data = points_grid %>% filter(!excluded),
    aes(x = rCMRGlc, y = Diff.pre),
    inherit.aes = FALSE, pch = 16, size = 1.8
  ) +
  # Excluded points: red triangles (matches add_points_and_labels convention)
  geom_point(
    data = points_grid %>% filter(excluded),
    aes(x = rCMRGlc, y = Diff.pre),
    inherit.aes = FALSE,
    shape = 24, fill = "firebrick2", color = "firebrick4", size = 3
  ) +
  facet_grid(structure_config ~ evolution_model) +
  coord_cartesian(xlim = c(10, 40), ylim = c(-3.5, 1.5)) +
  labs(
    title    = "Quadratic robustness: 4 structure configurations x 3 PGLS evolution models",
    subtitle = "Fit + 95% CI on included subset; excluded structures shown as red triangles",
    x = x_lab,
    y = y_lab
  ) +
  theme_paper +
...
# --- Save 12-panel figure ---
ggsave("figs/s3/all/plot_quadratic_comparison_12way.pdf",
       p_quad_compare_12way, width = 14, height = 12)
ggsave("figs/s3/all/plot_quadratic_comparison_12way.png",
       p_quad_compare_12way, width = 14, height = 12, dpi = 300)
ggsave("figs/s3/all/plot_quadratic_comparison_12way.jpg",
       p_quad_compare_12way, width = 14, height = 12, dpi = 300,
       device = "jpeg", bg = "white")

# --- Save 12-row stats table ---
dir.create("tables/s3/all", showWarnings = FALSE, recursive = TRUE)
write_xlsx(
  quad_grid %>% arrange(structure_config, evolution_model),
  "tables/s3/all/Table_S_quadratic_comparison_12way.xlsx"
)
