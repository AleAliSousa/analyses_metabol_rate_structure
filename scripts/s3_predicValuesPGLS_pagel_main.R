...
model_wide <- core_with_rCMRGlc %>%
  mutate(
    ModelGroup = case_when(
      grepl("brownian|\\bbm\\b", tolower(Model)) ~ "Pagel_main_model",
      grepl("pagel|lambda|\\bml\\b", tolower(Model)) ~ "Pagel's lambda (ML)",
      TRUE ~ as.character(Model)
    ),
    ModelGroup = factor(ModelGroup, levels = c("Pagel_main_model", "Pagel's lambda (ML)"))
  ) %>%
...
bm_cols <- paste0("Pagel_main_model__", metric_cols)
...
header_top <- c(
  "Structure",
  "Pagel_main_model", rep("", length(metric_cols) - 1),
...
diff_cols <- c(
  "Pagel_main_model__Diff.min",
  "Pagel_main_model__Diff.pre",
  "Pagel_main_model__Diff.max",
...
df_bm <- core_with_rCMRGlc %>%
  filter(Model == "Pagel's lambda (ML)") %>%
  transmute(
    Structure,
    rCMRGlc  = num(rCMRGlc),
...
y_lab <- "Difference from prediction (Pagel ML)"
