...
# --- Telencephalon vs non-telencephalon grouping, in same raw-name space as region_labels ---
# Telencephalon here includes cortical structures, hippocampus, amygdala, basal ganglia,
# telencephalic white matter, and internal capsule.
region_groups <- c(
  "LGN_Sousa"             = "Non-telencephalon",
  "Amygdala"              = "Telencephalon",
  "Pallidum"              = "Telencephalon",
  "NeoW_Frahm"            = "Telencephalon",
  "Total_insula_volume_L" = "Telencephalon",
  "Nucleus_subthalamicus" = "Non-telencephalon",
  "Capsula_interna"       = "Telencephalon",
  "Striatum"              = "Telencephalon",
  "ASG_Sousa"             = "Telencephalon",
  "NeoG_Frahm"            = "Telencephalon",
  "Mesencephalon"         = "Non-telencephalon",
  "Cerebellum"            = "Non-telencephalon",
  "Hippocampus"           = "Telencephalon"
)

# Defensive check: every analyzed structure must have exactly one anatomical group.
stopifnot(setequal(names(region_labels), names(region_groups)))

telencephalon_group_levels <- c("Telencephalon", "Non-telencephalon")
...
group_region <- function(x) {
  x_chr <- as.character(x)
  out <- unname(region_groups[x_chr])
  out[is.na(out)] <- "Unassigned"
  factor(out, levels = telencephalon_group_levels)
}

region_metadata <- tibble(
  Variable = target_cols,
  Structure = label_region(target_cols),
  RegionGroup = group_region(target_cols)
)

region_group_counts <- region_metadata %>%
  count(RegionGroup, name = "n_structures")
...
core_df$RegionGroup <- group_region(core_df$Variable)
...
final_df$RegionGroup <- group_region(final_df$Variable)
...
ggsave("figs/s3/telencephalon/plot_prediction_BM_vs_pagel.png", p1, width = 10, height = 7, dpi = 300)
ggsave("figs/s3/telencephalon/plot_standardized_prediction_error.png", p2, width = 10, height = 7, dpi = 300)
ggsave("figs/s3/telencephalon/plot_human_brain_mosaicism.png", p3, width = 12, height = 8, dpi = 300)
ggsave("figs/s3/telencephalon/plot_pagel_lambda_profile.png", p4, width = 12, height = 9, dpi = 300)

ggsave("figs/s3/telencephalon/plot_prediction_BM_vs_pagel.pdf", p1, width = 10, height = 7)
ggsave("figs/s3/telencephalon/plot_standardized_prediction_error.pdf", p2, width = 10, height = 7)
ggsave("figs/s3/telencephalon/plot_human_brain_mosaicism.pdf", p3, width = 12, height = 8)
ggsave("figs/s3/telencephalon/plot_pagel_lambda_profile.pdf", p4, width = 12, height = 9)
...
pdf("figs/s3/telencephalon/phylo_gls_region_plots.pdf", width = 7, height = 5)
...
dir.create("figs/s3/telencephalon/phylo_gls_region_jpgs", showWarnings = FALSE)
...
iwalk(
  ...
  function(p, region_label) {
    ...
    ggsave(
      filename = file.path(
        "figs/s3/telencephalon/phylo_gls_region_jpgs",
...
# 1) Column calculations
  mutate(
    ...
    RegionGroup = group_region(Variable),
...
# 3) Order rows by label order and model
  mutate(
    RegionGroup = factor(RegionGroup, levels = telencephalon_group_levels),
    Structure = factor(Structure, levels = label_region(target_cols))
  ) %>%
  arrange(RegionGroup, Structure, Model) %>%
...
write_csv(
  core_with_rCMRGlc,
  "checks/s3/telencephalon/core_with_rCMRGlc_predicted_volumes.csv"
)
...
model_wide <- core_with_rCMRGlc %>%
  ...
  select(RegionGroup, Structure, ModelGroup, all_of(metric_cols)) %>%
  pivot_wider(
    id_cols = c(RegionGroup, Structure),
...
shared_cols <- core_with_rCMRGlc %>%
  group_by(RegionGroup, Structure) %>%
  summarise(
    N = dplyr::first(N),
...
tbl_body <- model_wide %>%
  left_join(shared_cols, by = c("RegionGroup", "Structure")) %>%
  arrange(RegionGroup, Structure) %>%
  select(RegionGroup, Structure, all_of(bm_cols), all_of(ml_cols), N, rCMRGlc)
...
write_xlsx(print_ready_df, "tables/s3/telencephalon/Table 4 Predicted volumes for Brownian and Pagel's lambda models.xlsx", col_names = FALSE)

## -------------------------
## TO REPORT IN TEXT OR SUP: Correlation analysis between rCMRGlc and prediction error (Diff.pre)
## Analyses are run separately for telencephalon and non-telencephalon structures.
## -------------------------
...
write_csv(cor_pearson, "checks/s3/telencephalon/correlation_rCMRGlc_prediction_error_by_telencephalon_group_pearson.csv")
write_csv(cor_spearman, "checks/s3/telencephalon/correlation_rCMRGlc_prediction_error_by_telencephalon_group_spearman.csv")

## -------------------------
## MORE PLOTS 
## These analyses are stratified by telencephalon vs non-telencephalon.
## Neocortex white is shown but excluded from rCMRGlc fits as before.
## -------------------------
# ============================================================
# Data defined once
# Fits are stratified by RegionGroup.
# ============================================================
...
# ============================================================
# Plot: LOESS with Diff.min / Diff.max error bars, fit separately by RegionGroup
# ============================================================
...
p_loess_bars <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Fits are stratified by telencephalon group; Neocortex white is plotted but excluded from fits",
    x = x_lab,
    y = y_lab
  ) +
  theme_paper
...
p_poly_compare <- ggplot() +
  ...
  labs(
    title = "Polynomial fits by telencephalon group",
...
# ============================================================
# Plot: Quadratic fit with 95% confidence interval, fit separately by RegionGroup
# ============================================================
...
fit_quadratic_by_group <- function(df_group) {
  ...
  quad_pred <- cbind(
    ...
    as_tibble() %>%
    mutate(RegionGroup = group_name, .before = 1)
...
p_quad_ci <- ggplot() +
  ...
  labs(
    title = "Quadratic fits by telencephalon group",
...
ggsave("figs/s3/telencephalon/plot_loess_errorbars_BM_rCMRGlc_by_telencephalon_group.png", p_loess_bars, width = 9, height = 5, dpi = 300)
ggsave("figs/s3/telencephalon/plot_polynomial_comparison_BM_rCMRGlc_by_telencephalon_group.png", p_poly_compare, width = 11, height = 5.5, dpi = 300)
ggsave("figs/s3/telencephalon/plot_quadratic_CI_BM_rCMRGlc_by_telencephalon_group.png", p_quad_ci, width = 9, height = 5, dpi = 300)

ggsave("figs/s3/telencephalon/plot_loess_errorbars_BM_rCMRGlc_by_telencephalon_group.pdf", p_loess_bars, width = 9, height = 5)
ggsave("figs/s3/telencephalon/plot_polynomial_comparison_BM_rCMRGlc_by_telencephalon_group.pdf", p_poly_compare, width = 11, height = 5.5)
ggsave("figs/s3/telencephalon/plot_quadratic_CI_BM_rCMRGlc_by_telencephalon_group.pdf", p_quad_ci, width = 9, height = 5)
