...
#
# Keep this list aligned with s3_predicValuesPGLS_MERGED_variant.R.
# These are the 14 core Study 3 response regions plus its total-brain predictor.
# To exclude a variable from this QC run, simply comment out its line below.
# Do not add variables here unless they are used by the Study 3 script.
# ------------------------------------------------------------

compare_vars <- c(
  "Total_brain_net_volume",        # predictor: Total_brain_net_volume_Vol.mm3
...
crosswalk_missing <- crosswalk %>%
  filter(stephan_var %in% compare_vars) %>%
...
# Restrict the QC to variables actually used by Study 3.  The full crosswalk
# above is retained as documentation, but variables not listed in compare_vars
# are ignored completely by the comparisons and summaries below.
unknown_compare_vars <- setdiff(compare_vars, crosswalk$stephan_var)
if (length(unknown_compare_vars) > 0) {
  warning("compare_vars contains name(s) not present in the crosswalk: ",
          paste(unknown_compare_vars, collapse = ", "))
}

crosswalk <- crosswalk %>%
  filter(stephan_var %in% compare_vars,
         wide_var %in% wide_names,
         stephan_var %in% stephan_names) %>%
  mutate(.order = match(stephan_var, compare_vars)) %>%
  arrange(.order) %>%
  select(-.order)
...
# Variable coverage
#
# Deliberately report only requested Study 3 variables. Other columns in either
# source file are irrelevant to this QC and are not listed as "unmapped".
# ------------------------------------------------------------

requested_crosswalk <- tribble(
  ~stephan_var, ~wide_var,
...
missing_requested <- requested_crosswalk %>%
  mutate(
    missing_from_wide = !(wide_var %in% wide_names),
    missing_from_stephan = !(stephan_var %in% stephan_names)
  ) %>%
  filter(missing_from_wide | missing_from_stephan)

write_csv(missing_requested,
          file.path(out_dir, "requested_study3_variables_missing.csv"))
