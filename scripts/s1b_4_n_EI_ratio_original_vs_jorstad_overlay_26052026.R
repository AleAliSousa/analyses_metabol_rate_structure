...
###################################################################
# Original broad E:I category definition from the previous script
###################################################################
# This keeps the previous broad ratio intact:
#   broad_original_EI = all Excitatory_projection superclusters /
#                       all Inhibitory_interneuron superclusters
# Medium spiny neurons are retained as a separate diagnostic denominator
# but are not included in the original broad denominator unless explicitly
# using broad_with_MSN_EI.
...
original_wide <- calculate_region_props(
  ...
  mutate(
    ...
    p_original_Inhibitory_all_with_MSN = p_original_Inhibitory_interneuron + p_original_Inhibitory_projection_MSN,
    EI_ratio = if_else(
    ...
    log2_EI_ratio_with_MSN = log2(EI_ratio_with_MSN),
...
# Plot helpers
###################################################################
plot_df_original <- comparison_df %>%
  filter(EI_definition == "Original broad E:I, all regions")

plot_df_jorstad <- comparison_df %>%
  filter(EI_definition == "Jorstad-like cortical E:I")

# Overlay: original all-region background with Jorstad-like cortical subset on top.
p_overlay <- ggplot() +
  geom_point(
...
if (has_ggrepel) {
  p_overlay <- p_overlay +
    ggrepel::geom_text_repel(
      data = plot_df_jorstad,
      aes(x = log2_EI_ratio, y = rcmr_value, label = anatomy_group),
      color = "#0072B2",
      size = 3.0,
      max.overlaps = Inf,
...
p_facet <- ggplot(comparison_df, aes(x = log2_EI_ratio, y = rcmr_value)) +
  geom_point(aes(color = cortex_call), alpha = 0.8, size = 2.6) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  facet_wrap(~ EI_definition, scales = "free_x") +
  scale_color_manual(values = c("Cortex" = "#0072B2", "Non-cortex" = "grey60"), na.value = "grey70") +
  labs(
    title = "E:I definition comparison",
    subtitle = "The Jorstad-like definition is restricted to cortical regions and cortical E/I supercluster families.",
    x = "log2(E:I ratio)",
    y = "rCMRGlc (µmol/100 g/min.)",
    color = "Region class"
  ) +
...
p_original_highlight <- ggplot(plot_df_original, aes(x = log2_EI_ratio, y = rcmr_value)) +
  geom_point(aes(color = cortex_call), alpha = 0.85, size = 2.8) +
  geom_smooth(aes(group = 1), method = "lm", se = TRUE, color = "grey35") +
  geom_smooth(
    data = plot_df_original %>% filter(is_cortex),
    aes(group = 1),
    method = "lm",
    se = TRUE,
    color = "#0072B2",
  ...
  scale_color_manual(values = c("Cortex" = "#0072B2", "Non-cortex" = "grey60"), na.value = "grey70") +
  labs(
    title = "Original broad E:I with cortical regions highlighted",
    subtitle = "Grey line: all regions. Blue line: cortical regions only, using the original broad E/I definition.",
    x = "log2(original broad E:I ratio)",
    y = "rCMRGlc (µmol/100 g/min.)",
    color = "Region class"
  ) +
  theme_classic(base_size = 13)
...
p_overlay_raw <- ggplot() +
  ...
  labs(
    title = "Raw E:I ratio overlay",
    subtitle = "Grey: original broad all-region E:I. Blue: Jorstad-like cortex-only E:I.",
    x = "E:I ratio",
    y = "rCMRGlc (µmol/100 g/min.)"
  ) +
  theme_classic(base_size = 13)
