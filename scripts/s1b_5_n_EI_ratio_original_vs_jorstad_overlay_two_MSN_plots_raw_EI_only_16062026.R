...
original_wide <- calculate_region_props(
  ...
  mutate(
    ...
    # Legacy alias: this retains the previous MSN-excluded definition.
    ...
    delta_EI_with_MSN_minus_no_MSN =
      EI_ratio_with_MSN - EI_ratio_no_MSN
  )
...
###################################################################
safe_spearman <- function(df, variable = "EI_ratio") {
  x <- df[[variable]]
  y <- df$rcmr_value
...
spearman_tbl <- comparison_df %>%
  ...
  group_modify(~ safe_spearman(.x, "EI_ratio")) %>%
  ungroup() %>%
...
lm_summary_one <- function(df, variable = "EI_ratio") {
...
lm_tbl <- comparison_df %>%
  group_by(EI_definition) %>%
  group_modify(~ lm_summary_one(.x, "EI_ratio")) %>%
  ungroup() %>%
  mutate(p_adj_BH = p.adjust(p_value, method = "BH"))
