...
# so ~51 of its rows are not tips of species.nwk. The tree join already dropped
# them, silently; this just makes the drop explicit and reports it.
#
# To analyse a different set, set TAXA to any character vector of species, e.g.
#   TAXA <- tr$tip.label                                  # default, all tips
#   TAXA <- setdiff(tr$tip.label, "Homo_sapiens")         # drop humans
#   TAXA <- grep("^(Homo|Pan|Gorilla|Pongo)_", tr$tip.label, value = TRUE)
#   TAXA <- unique(data_clean$Species)                    # no filtering
TAXA <- tr$tip.label

dropped_taxa <- setdiff(unique(data_clean$Species), TAXA)
data_clean   <- data_clean[data_clean$Species %in% TAXA, , drop = FALSE]

message("Taxon selection: ", nrow(data_clean), " rows kept, ",
        length(dropped_taxa), " out-of-scope labels dropped.")

# Tips with no row at all. Name-level fixes belong upstream, not here.
absent <- setdiff(tr$tip.label, unique(data_clean$Species))
if (length(absent))
  message("Tips with no row in volumes_wide_select.csv (", length(absent), "): ",
          paste(absent, collapse = ", "))

# Equivalent total-brain predictor in volumes_wide_select.csv.
data <- data_clean %>%
  mutate(
    Preferred_brain_volume = Total_brain_net_volume_Vol.mm3
  )

# Sanity check: species without total brain volume simply drop out of each fit.
n_missing_pred <- sum(is.na(data$Preferred_brain_volume))
if (n_missing_pred > 0) {
  warning(n_missing_pred, " species lack Total_brain_net_volume_Vol.mm3 ",
...
# ============================================================
region_labels <- c(
  "Corpus_geniculatum_laterale_Vol.mm3" = "Corpus geniculatum laterale",
  "Amygdala_Vol.mm3"                    = "Amygdala",
  "Pallidum_Vol.mm3"                    = "Pallidum",
  "Neocortex_white_matter_Vol.mm3"      = "Neocortex white",
  "Insula_left_Vol.mm3"                 = "Insular cortex (grey)",
  "Nucleus_subthalamicus_Vol.mm3"       = "Nucleus subthalamicus Luysi",
  "Capsula_interna_Vol.mm3"             = "Capsula interna",
  "Striatum_Vol.mm3"                    = "Striatum",
  "Area_striata_grey_matter_Vol.mm3"    = "Area striata grey",
  "Neocortex_grey_matter_Vol.mm3"       = "Neocortex grey",
  "Mesencephalon_Vol.mm3"               = "Mesencephalon",
  "Cerebellum_Vol.mm3"                  = "Cerebellum",
  "Hippocampus_Vol.mm3"                 = "Hippocampus",
  "Lateral_cerebellar_nuclei_Vol.mm3"   = "Nucleus dentatus cerebelli"
)
