...
# ============================================================
# rCMRGlc vs prediction-error plots (fit excludes Neocortex grey)
# ============================================================
...
df_bm <- core_with_rCMRGlc %>%
  ...
  transmute(Structure, rCMRGlc = num(rCMRGlc),
            ...
            excluded = Structure == "Neocortex grey") %>%
...
add_points_and_labels <- list(
  ...
  geom_text(data = df_neo, aes(rCMRGlc, Diff.pre, label = "Neocortex grey (excluded)"),
...
p_loess_bars <- ggplot() +
  ...
  labs(title = "LOESS fit with error bars",
       subtitle = "Fit excludes Neocortex grey; plotted points include it", x = x_lab, y = y_lab) +
  theme_paper
p_loess_bars
...
p_poly_compare <- ggplot() +
  ...
  labs(title = "Polynomial fits, degree 1 to 5",
       subtitle = paste0("Models exclude Neocortex grey; best fit: degree ", best_degree),
       x = x_lab, y = y_lab, color = NULL) +
  theme_paper + theme(legend.position = "right")
p_poly_compare

# ---- Quadratic fit with 95% CI ----
quad_fit <- lm(Diff.pre ~ poly(rCMRGlc, 3, raw = TRUE), data = df_fit)
...
#   Neocortex grey is always excluded from the rCMRGlc regressions and

#   is plotted afterwards as a distinct bright-blue square.

# ============================================================

always_excluded_from_fit <- "Neocortex grey"
...
# Per-cell point data.
# Neocortex grey is always excluded from the regression fits, but plotted afterwards
# as its own point class. White-matter exclusions are panel-specific.
points_grid <- map_dfr(structure_levels, function(sc_name) {
  ...
  diff_pre_with_rCMRGlc %>%
    mutate(
      structure_config = sc_name,
      neocortex_grey = Structure == always_excluded_from_fit,
...
p_linear_compare_12way <- ggplot() +
  ...
  # Neocortex grey: always excluded from regression and overlaid afterwards
  geom_point(
    data = filter(points_grid, neocortex_grey),
    aes(rCMRGlc, Diff.pre),
    inherit.aes = FALSE,
    shape = 22,
    fill = "deepskyblue1",
  ...
  labs(
    ...
    subtitle = "Linear fit + 95% CI on included subset; white-matter exclusions = red triangles; Neocortex grey = bright blue square",
    x = x_lab,
    y = y_lab
  ) +
  theme_paper +
