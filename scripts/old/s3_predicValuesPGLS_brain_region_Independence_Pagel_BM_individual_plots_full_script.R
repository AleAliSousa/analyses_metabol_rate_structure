...
# --- Robust label helper to convert raw variable names to display labels (safe with factors) ---
...
# --- Helper: individual structure GLS/PGLS plot for one model
# Models supported:
#   Independence: Pagel's lambda fixed at 0
#   PagelML:      Pagel's lambda estimated by ML
#   BM:           Brownian motion, equivalent to lambda fixed at 1
plot_region_gls <- function(region,
                            data,
                            tr,
                            human = "Homo_sapiens",
                            model = c("Independence", "PagelML", "BM")) {
  ...
  # GLS/PGLS correlation structure
  if (model == "Independence") {
    model_label <- "Independence (lambda = 0)"
    model_short <- "Independence, λ=0"
    lambda_txt  <- "λ = 0"
    cor_struct  <- corPagel(0, form = ~Species, phy = pp$tr2, fixed = TRUE)
  }
  
  if (model == "PagelML") {
    model_label <- "Pagel's lambda ML"
    model_short <- "Pagel ML"
    cor_struct  <- corPagel(1, form = ~Species, phy = pp$tr2, fixed = FALSE)
  }
  
  if (model == "BM") {
    model_label <- "Brownian Motion"
    model_short <- "BM, λ=1"
    lambda_txt  <- "λ = 1"
  ...
  if (is.null(fit)) {
    warning("Skipping ", region, " ", model, ": fit_gls_safe() returned NULL")
    return(NULL)
  }
  
  # Extract estimated lambda for Pagel ML subtitle
  if (model == "PagelML") {
    lambda_est <- as.numeric(coef(fit$modelStruct, unconstrained = FALSE))
    lambda_txt <- paste0("estimated λ = ", sprintf("%.3f", lambda_est))
  }
  ...
  plot_df <- pp$d_full %>%
    mutate(is_human = Species == pp$human)
  ...
  ggplot() +
    ...
    theme_bw() +
    labs(
      title = paste0("GLS/PGLS (", model_short, "): ", pretty_region, " vs Rest of Brain"),
      subtitle = paste0(
        "Solid = non-human range; dashed = extrapolation to human. ",
        "Human shown as triangle; fit excludes human; ", lambda_txt, "."
      ),
...
profile_list <- purrr::map(target_cols, function(vn) {
  pp <- prep_structure(vn, data = data, tr = tr, human = "Homo_sapiens")
...
df_profile <- dplyr::bind_rows(purrr::map(profile_list, "scan"))
df_mle     <- dplyr::bind_rows(purrr::map(profile_list, "mle"))
...
dir.create("figs/s3/all", showWarnings = FALSE, recursive = TRUE)

ggsave("figs/s3/all/plot_prediction_BM_vs_pagel.png", p1, width = 7, height = 5, dpi = 300)
ggsave("figs/s3/all/plot_standardized_prediction_error.png", p2, width = 7, height = 5, dpi = 300)
ggsave("figs/s3/all/plot_human_brain_mosaicism.png", p3, width = 7, height = 5, dpi = 300)
ggsave("figs/s3/all/plot_pagel_lambda_profile.png", p4, width = 7, height = 5, dpi = 300)

ggsave("figs/s3/all/plot_prediction_BM_vs_pagel.pdf", p1, width = 10, height = 7)
ggsave("figs/s3/all/plot_standardized_prediction_error.pdf", p2, width = 10, height = 7)
ggsave("figs/s3/all/plot_human_brain_mosaicism.pdf", p3, width = 10, height = 7)
ggsave("figs/s3/all/plot_pagel_lambda_profile.pdf", p4, width = 10, height = 7)


######### INDIV SECTION PLOTS PT 2 START
## -------------------------
## PLOT V: individual structure plots for Independence, Pagel ML, and BM
## -------------------------

# This block intentionally uses separate objects for each model.
# Do not use a single flat object named `plots`, because that makes it unclear
# whether it contains BM, Pagel, Independence, or some mixture.

# Output directories
individual_plot_dir <- "figs/s3/all"
individual_jpg_dir  <- file.path(individual_plot_dir, "phylo_gls_region_jpgs")

dir.create(individual_plot_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(individual_jpg_dir,  showWarnings = FALSE, recursive = TRUE)

# File-safe name helper
...
# Build one named list of plots for a given model
build_region_plots <- function(model_code) {
  target_cols %>%
...
# -------------------------
# Build Independence plots, lambda fixed at 0
# -------------------------
plots_independence_raw <- build_region_plots("Independence")
failed_regions_independence <- names(plots_independence_raw)[purrr::map_lgl(plots_independence_raw, is.null)]
plots_independence <- purrr::compact(plots_independence_raw)

# -------------------------
# Build Pagel's lambda ML plots
# -------------------------
plots_pagel_raw <- build_region_plots("PagelML")
failed_regions_pagel <- names(plots_pagel_raw)[purrr::map_lgl(plots_pagel_raw, is.null)]
plots_pagel <- purrr::compact(plots_pagel_raw)

# -------------------------
# Build BM plots, lambda fixed at 1
# -------------------------
plots_bm_raw <- build_region_plots("BM")
failed_regions_bm <- names(plots_bm_raw)[purrr::map_lgl(plots_bm_raw, is.null)]
plots_bm <- purrr::compact(plots_bm_raw)

# Optional nested object for convenience only
plots_by_model <- list(
  Independence_lambda0 = plots_independence,
  Pagel_ML = plots_pagel,
  BM_lambda1 = plots_bm
)
...
cat("Independence lambda=0 plots: ", length(plots_independence), "\n", sep = "")
cat("Pagel ML plots: ", length(plots_pagel), "\n", sep = "")
cat("BM lambda=1 plots: ", length(plots_bm), "\n", sep = "")

cat("\nFailed Independence lambda=0 regions:\n")
print(failed_regions_independence)

cat("\nFailed Pagel ML regions:\n")
print(failed_regions_pagel)

cat("\nFailed BM regions:\n")
print(failed_regions_bm)

cat("\nIndependence lambda=0 plot names:\n")
print(names(plots_independence))

cat("\nPagel ML plot names:\n")
print(names(plots_pagel))

cat("\nBM plot names:\n")
print(names(plots_bm))

# -------------------------
# Print plots interactively to RStudio plot pane / active graphics device
# -------------------------
cat("\nPrinting Independence lambda=0 plots...\n")
purrr::walk(plots_independence, print)

cat("\nPrinting Pagel ML plots...\n")
purrr::walk(plots_pagel, print)

cat("\nPrinting BM lambda=1 plots...\n")
purrr::walk(plots_bm, print)

# -------------------------
# Save each model to its own multipage PDF
# -------------------------
pdf(file.path(individual_plot_dir, "phylo_gls_region_plots_Independence_lambda0.pdf"), width = 7, height = 5)
purrr::walk(plots_independence, print)
dev.off()

pdf(file.path(individual_plot_dir, "phylo_gls_region_plots_Pagel_ML.pdf"), width = 7, height = 5)
purrr::walk(plots_pagel, print)
dev.off()

pdf(file.path(individual_plot_dir, "phylo_gls_region_plots_BM_lambda1.pdf"), width = 7, height = 5)
purrr::walk(plots_bm, print)
dev.off()

# -------------------------
# Save one presentation-ready JPG per structure per model
# -------------------------
save_region_jpgs <- function(plot_list, model_prefix) {
  purrr::iwalk(
    plot_list,
    function(p, region_label) {
      ggsave(
        filename = file.path(
          individual_jpg_dir,
          paste0("phylo_gls_", model_prefix, "_", safe_name(region_label), ".jpg")
        ),
        plot = p,
        width = 13.333,
        height = 7.5,
...
save_region_jpgs(plots_independence, "Independence_lambda0")
save_region_jpgs(plots_pagel,       "Pagel_ML")
save_region_jpgs(plots_bm,          "BM_lambda1")

# -------------------------
# Final saved-file checks
# -------------------------
cat("\nSaved individual plot PDFs:\n")
print(list.files(individual_plot_dir, pattern = "^phylo_gls_region_plots_.*\\.pdf$", full.names = TRUE))

cat("\nSaved Independence lambda=0 JPGs:\n")
print(list.files(individual_jpg_dir, pattern = "^phylo_gls_Independence_lambda0_.*\\.jpg$", full.names = TRUE))

cat("\nSaved Pagel ML JPGs:\n")
print(list.files(individual_jpg_dir, pattern = "^phylo_gls_Pagel_ML_.*\\.jpg$", full.names = TRUE))

cat("\nSaved BM lambda=1 JPGs:\n")
print(list.files(individual_jpg_dir, pattern = "^phylo_gls_BM_lambda1_.*\\.jpg$", full.names = TRUE))
...
dir.create("tables/s3/all", showWarnings = FALSE, recursive = TRUE)
dir.create("checks/s3/all", showWarnings = FALSE, recursive = TRUE)

write_csv(
  core_with_rCMRGlc,
  "checks/s3/all/core_with_rCMRGlc_predicted_volumes.csv"
)
...
write_xlsx(print_ready_df, "tables/s3/all/Table 4 Predicted volumes for Brownian and Pagel's lambda models.xlsx", col_names = FALSE)
...
ggsave("figs/s3/all/plot_loess_errorbars_BM_rCMRGlc.png", p_loess_bars, width = 7, height = 5, dpi = 300)
ggsave("figs/s3/all/plot_polynomial_comparison_BM_rCMRGlc.png", p_poly_compare, width = 9, height = 5.5, dpi = 300)
ggsave("figs/s3/all/plot_quadratic_CI_BM_rCMRGlc.png", p_quad_ci, width = 7, height = 5, dpi = 300)

ggsave("figs/s3/all/plot_loess_errorbars_BM_rCMRGlc.pdf", p_loess_bars, width = 7, height = 5)
ggsave("figs/s3/all/plot_polynomial_comparison_BM_rCMRGlc.pdf", p_poly_compare, width = 9, height = 5.5)
ggsave("figs/s3/all/plot_quadratic_CI_BM_rCMRGlc.pdf", p_quad_ci, width = 7, height = 5)
