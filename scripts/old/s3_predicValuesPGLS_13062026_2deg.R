...
#   mu_source = "trait" so the (Exclude Neocortex grey, Brownian) cell

#   reproduces the published fit above, now with degree = 3.
...
# Per-cell point data
points_grid <- map_dfr(structure_levels, 
function(sc_name) {
  excl <- unique(c(always_excluded_from_fit,structure_configs[[sc_name]])) #change this to make neocortex different color
  diff_pre_with_rCMRGlc %>%
    mutate(
      structure_config = sc_name,
      excluded = Structure %in% structure_configs
    )
})
...
p_linear_compare_12way <- ggplot() +
  ...
  #   size = 3
  #  ) 
# excluded (except Neocortex grey)
geom_point(
  data = filter(points_grid, excluded & Structure != "Neocortex grey"),
  aes(rCMRGlc, Diff.pre),
  inherit.aes = FALSE,
  shape = 24,
  fill = "firebrick2",
  ...
  # Neocortex grey (excluded, but distinct)
  geom_point(
    data = filter(points_grid, excluded & Structure == "Neocortex grey"),
    aes(rCMRGlc, Diff.pre),
    inherit.aes = FALSE,
    shape = 21,          # different symbol
    fill = "blue",
    color = "darkblue",
    size = 3
) +
  ...
  labs(
    ...
    subtitle = "Linear fit + 95% CI on included subset; excluded structures as red triangles",
    x = x_lab,
    y = y_lab
  ) +
  theme_paper +
