...
#   add_phylo_mu = TRUE returns the conditional phylogenetic prediction used by
#     the prediction-error/CI plots.
#   add_phylo_mu = FALSE returns the fitted GLS line prediction only; this is the
#     calculation used for the mosaicism plots so their sign matches the
#     "Phylogenetic GLS (BM): ..." line plots exactly.
...
# ============================================================
# PLOT III DATA (one pass, no CI; fitted-line prediction)
# ============================================================
# The mosaicism plots now use add_phylo_mu = FALSE so the human prediction is
# the same fitted GLS line value shown in the "Phylogenetic GLS (BM): ..." plots.
# The model is fit with natural logs via log(y) ~ log(Rest_of_Brain); the plot
# axes are displayed as log10 values. Because both are monotonic transforms, the
# above/below-line direction is identical on either logged scale.
...
plot_mosaicism <- function(df, title, subtitle) {
  ...
  ggplot(df, aes(x = reorder(VarLabel, PropDiff), y = PropDiff, color = Direction)) +
    ...
    labs(title = title, subtitle = subtitle,
         y = "Deviation from fitted-line prediction (%)", x = "Brain Structure", color = "Direction") +
...
p3 <- plot_mosaicism(
  ...
  subtitle = "Predictor = Rest of Brain (Total - Structure). Predictions use the same fitted-line values as the PGLS plots."
)
p3
...
p3_pagel_bm <- plot_mosaicism(
  ...
  subtitle = "Models shown = Pagel's ML and Brownian. Predictions use the same fitted-line values as the PGLS plots."
)
p3_pagel_bm
...
p3_bm_only <- plot_mosaicism(
  ...
  subtitle = "Model shown = Brownian. Predictions use the same fitted-line values as the BM PGLS plots."
)
p3_bm_only
...
p3_requested_versions <- ggplot(
  ...
  labs(title = "Human Brain Mosaicism (Corrected for Part-Whole): All Requested Views",
       subtitle = "Includes the original all-model view, Pagel's ML + Brownian, and Brownian-only view; predictions are fitted-line values matching the PGLS plots.",
       y = "Deviation from fitted-line prediction (%)", x = "Brain Structure", color = "Direction") +
...
# the human triangle with the fitted line in the individual PGLS plots.
bm_mosaicism_alignment_check <- final_df %>%
  filter(as.character(Model) == "Brownian (λ=1)") %>%
  transmute(
    Structure = VarLabel,
    Observed,
    Predicted_from_fitted_GLS_line = Predicted,
    Observed_log10,
    Predicted_log10,
    Log10Diff,
    PropDiff,
...
write.csv(bm_mosaicism_alignment_check,
          file.path(CONFIG$dir_checks, "bm_mosaicism_matches_phylo_gls_plot_alignment_check.csv"),
          row.names = FALSE)
...
build_region_gls_plot_data <- function(region, data, tr, human = CONFIG$human) {
  ...
  list(region = region, pretty_region = pretty_region, pred_df = pred_df, plot_df = plot_df)
}
...
make_region_gls_plot <- function(plot_data) {
  ...
  ggplot() +
    ...
    labs(title = paste0("Phylogenetic GLS (BM): ", pretty_region, " vs Rest of Brain"),
         subtitle = "Model fit: natural-log GLS; axes show log10 values. Solid = non-human range; dashed = extrapolation. Human = triangle; fit excludes human.",
...
p5_combined_gls <- ggplot() +
  ...
  theme_bw() +
  labs(title = "Phylogenetic GLS (BM): All Brain Structures vs Rest of Brain",
       subtitle = "Model fit: natural-log GLS; axes show log10 values. Solid = non-human range; dashed = extrapolation. Human = triangle; fit excludes human.",
       x = "log10(Rest of Brain [Total brain - structure])",
