...
model_wide <- core_with_rCMRGlc %>%
  mutate(
    ModelGroup = case_when(
      grepl("brownian|\\bbm\\b", tolower(Model)) ~ "Lambda0_main_model",
      grepl("pagel|lambda|\\bml\\b", tolower(Model)) ~ "Pagel's lambda (ML)",
      TRUE ~ as.character(Model)
    ),
    ModelGroup = factor(ModelGroup, levels = c("Lambda0_main_model", "Pagel's lambda (ML)"))
  ) %>%
...
bm_cols <- paste0("Lambda0_main_model__", metric_cols)
...
header_top <- c(
  "Structure",
  "Lambda0_main_model", rep("", length(metric_cols) - 1),
...
diff_cols <- c(
  "Lambda0_main_model__Diff.min",
  "Lambda0_main_model__Diff.pre",
  "Lambda0_main_model__Diff.max",
...
y_lab <- "Difference from prediction (λ=0)"
