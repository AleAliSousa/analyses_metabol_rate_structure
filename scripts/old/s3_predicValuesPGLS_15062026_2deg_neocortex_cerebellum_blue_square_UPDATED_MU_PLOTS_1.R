...
make_region_gls_plot <- function(plot_data) {
  ...
  # Equation of the line as actually plotted:
  # y-axis: log10(region volume)
  # x-axis: log10(Rest of Brain)
  eqn_df <- pred_df |>
    filter(is.finite(x_log10), is.finite(fit_log10)) |>
    distinct(x_log10, fit_log10)
  
  eqn_fit <- lm(fit_log10 ~ x_log10, data = eqn_df)
  eqn_coef <- coef(eqn_fit)
  
  intercept <- unname(eqn_coef[1])
  slope <- unname(eqn_coef[2])
  
  fmt_num <- function(z, digits = 3) {
    formatC(z, format = "f", digits = digits)
  }
  
  fmt_signed <- function(z, digits = 3) {
    paste0(
      ifelse(z >= 0, " + ", " - "),
      fmt_num(abs(z), digits = digits)
    )
  }
...
p5_combined_gls <- ggplot() +
  ...
  labs(title = "Phylogenetic GLS (BM + human mu): All Brain Structures vs Rest of Brain",
       subtitle = "Axes show log10 values; human = triangle; predicted human value = x.",
       x = "log10(Rest of Brain [Total brain - structure])",
