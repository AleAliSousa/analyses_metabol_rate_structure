...
p3_pagel_bm <- plot_mosaicism(
  ...
  subtitle = "Models shown = Pagel's ML and Brownian. Predictor = Rest of Brain (Total - Structure)."
)
p3_pagel_bm
...
p3_bm_only <- plot_mosaicism(
  ...
  subtitle = "Model shown = Brownian. Predictor = Rest of Brain (Total - Structure)."
)
p3_bm_only
...
p3_requested_versions <- ggplot(
  ...
  labs(title = "Human Brain Mosaicism (Corrected for Part-Whole): All Requested Views",
       subtitle = "Includes the original all-model view, Pagel's ML + Brownian, and Brownian-only view.",
       y = "Deviation from Prediction (%)", x = "Brain Structure", color = "Direction") +
...
make_region_gls_plot <- function(plot_data) {
  ...
  ggplot() +
    ...
    labs(title = paste0("Phylogenetic GLS (BM): ", pretty_region, " vs Rest of Brain"),
         subtitle = "Axes show log10 values. Solid = non-human range; dashed = extrapolation. Human = triangle; fit excludes human.",
