...
# Helper to convert raw variable names → pretty names
pretty_var <- function(x) unname(label_map[x])
...
# --- 1) Robust label helper (safe with factors) ---
...
# --- 2) Ensure canonical order on Variable (matches target_cols) ---
core_df$Variable <- factor(core_df$Variable, levels = target_cols)

# --- 3) Compute VarLabel for display ---
core_df <- core_df %>%
  mutate(VarLabel = pretty_var(Variable))

# --- 4) Recompute N at the end (species used per structure after clean + excluding human) ---
#     We recompute N here so you don't have to modify the modeling function.
count_N_for <- function(var_name, data, tr, human = "Homo_sapiens") {
  if (!var_name %in% names(data)) return(NA_integer_)
  d1 <- data[, c("Species", "Brain_weight", var_name)]
  d1 <- as.data.frame(na.omit(d1))
  if (!human %in% d1$Species) return(NA_integer_)
  rownames(d1) <- d1$Species
  clean <- clean.data(d1, tr)
  tr1 <- clean$tree
  data2 <- clean$data
...
N_by_var <- tibble(
  ...
  N = purrr::map_int(target_cols, ~ count_N_for(.x, data = data, tr = tr))
)

# Join N onto core_df (joins by Variable)
core_df <- core_df %>%
  left_join(N_by_var, by = "Variable")

# --- 5) OPTIONAL: add any derived columns you may want later ---
core_df <- core_df %>%
  mutate(
...
# --- 6) Reorder/select columns to your preferred layout (and include N) ---
# Desired header:
...
# --- 7) Order rows by intended label order, then Model ---
