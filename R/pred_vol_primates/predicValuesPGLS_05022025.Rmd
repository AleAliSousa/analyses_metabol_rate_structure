...
## -------------------------
## TABLE I: Predicted values + CIs (one pass, with CI) and rCMRGIc
## -------------------------
...
# Drop rows 1–4 and keep only the relevant columns; ensure rCMRGIc is numeric and rounded to 1 decimal place
sup_clean <- sup_tbl_raw[-c(1:4), ] %>%
  ...
  mutate(
    rCMRGIc = sprintf("%.1f", readr::parse_number(as.character(rCMRGIc)))
  )
...
shared_cols <- core_with_rCMRGIc %>%
  group_by(Structure) %>%
  summarise(
    N = dplyr::first(N),
    rCMRGIc = dplyr::first(rCMRGIc),
    .groups = "drop"
  )
...
tbl_body <- model_wide %>%
  ...
  arrange(Structure) %>%
  select(Structure, all_of(bm_cols), all_of(ml_cols), N, rCMRGIc)

# from your existing tbl_body (Structure + BM block + ML block + N + rCMRGIc)
...
write.table(print_ready_df, "print_ready_table.tsv",
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write_xlsx(print_ready_df, "print_ready_table.xlsx", col_names = FALSE)

## -------------------------
## TO REPORT: Correlation analysis between rCMRGIc and prediction error (Diff.pre) across structures
## -------------------------
...
cor_df <- tbl_body %>%
  transmute(
    Structure,
    rCMRGIc = parse_number(as.character(rCMRGIc)),
