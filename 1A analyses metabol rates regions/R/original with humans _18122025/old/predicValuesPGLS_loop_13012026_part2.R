...
## 2nd Standardised PLOT (all values normalized 0 = min in dtaaset for that structire; 1 is max)
...
library(ape)
library(ggplot2)
library(dplyr) # Added for easy grouping/scaling
...
# Helper: Transform VCV by Lambda
transform_vcv_lambda <- function(tree, lambda) {
  v <- vcv(tree, corr = TRUE)
  diag_vals <- diag(v)
  v <- v * lambda      
