...
for (var_name in target_cols) {
##### TEMP
# ---- DEBUG SWITCH (paste right after: for (var_name in target_cols) { ) ----
DEBUG_VAR <- "NeoW_Frahm"
DEBUG <- identical(var_name, DEBUG_VAR)

dbg <- function(...) if (DEBUG) cat(..., "\n")
# --------------------------------------------------------------------------
##### /TEMP
  if (!var_name %in% names(data)) next
  # A. PREPARE DATA
...
##### TEMP
if (DEBUG) {
  hs <- d1[d1$Species == "Homo_sapiens", ]
  dbg("Rows after NA omit:", nrow(d1))
  if (nrow(hs) == 1) {
    dbg("Human Brain_weight:", hs$Brain_weight)
    dbg("Human", var_name, ":", hs[[var_name]])
    dbg("Human Rest_of_Brain:", hs$Rest_of_Brain)
  } else {
    dbg("Human rows found:", nrow(hs))
  }
  dbg("Any Rest_of_Brain <= 0:", any(d1$Rest_of_Brain <= 0))
  dbg("Any response <= 0:", any(d1[[var_name]] <= 0))
}
##### /TEMP
...
##### TEMP
if (DEBUG) {
  dbg("Rows after ROB>0 filter:", nrow(d1))
  dbg("Human still present after ROB filter?:", "Homo_sapiens" %in% d1$Species)
}
##### /TEMP
  rownames(d1) <- d1$Species
...
##### TEMP REPLACE
  ...
  error = function(e) {
    if (DEBUG) {
      dbg("GLS FAILED for model:", mod$name)
      dbg("Error:", conditionMessage(e))
    }
    return(NULL)
  }
)
