...
plot3_df$VarLabel <- pretty_var(plot3_df$Variable)
# --- PLOT ---
ggplot(plot3_df, aes(x = reorder(VarLabel, PropDiff), y = PropDiff, color = Direction)) +
...
# ===============================
#### TABLE OUTPUT (run at the end)
# ===============================

# --- 1) Ensure canonical order on Variable (matches target_cols) ---
core_df$Variable <- factor(core_df$Variable, levels = target_cols)

# --- 2) Compute VarLabel for display ---
core_df <- core_df %>%
  mutate(VarLabel = pretty_var(Variable))

# --- 3) OPTIONAL: add any derived columns you may want later ---
core_df <- core_df %>%
  mutate(
    # Example extras (remove if not needed or extend as you wish):
    CI_width = Upper - Lower,
    Within_CI = Observed >= Lower & Observed <= Upper,
    PropDiff = (Observed - Predicted) / Predicted   # same metric used in Plot III
  )

# --- 4) Reorder/select columns to your preferred layout (and include N) ---
# Desired header:
# VarLabel, Model, Lambda, 95% CI min, Predicted, 95% CI max, Observed, N
core_df_out <- core_df %>%
  transmute(
    Structure = VarLabel,
    Model,
    ...
    Observed,
    N
    # If you want the optional columns as well, add them here:
    # , CI_width, Within_CI, PropDiff
  )

# --- 5) Order rows by intended label order, then Model ---
desired_label_order <- pretty_var(target_cols)
core_df_out <- core_df_out %>%
  mutate(Structure = factor(Structure, levels = desired_label_order)) %>%
  arrange(Structure, Model)

# ---  6) Limit significant figures
#    Adjust these digits if you prefer different precision later.
digits_lambda <- 3
digits_vals   <- 5   # Predicted, CIs, Observed
# Make N a clean integer
core_df_out <- core_df_out %>%
  dplyr::mutate(
...
# --- 7) Write the table ---
write.csv(core_df_out, "core_df_out.csv", row.names = FALSE)

############################################################
# Add rCMRGIc values to core_df_out.csv by matching Structure
# and preserve original CI column names
############################################################
...
# Load packages -------------------------------------------------------------
library(readxl)   # for reading the xlsx file
library(dplyr)    # for joins and data wrangling

# 1. Read core_df_out.csv, preserving column names --------
core_df <- read.csv(
  "core_df_out.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE   # <-- keep names like "95% CI min"
)

# 2. Read Supplementary Table 1 from its full path --------
...
# 2a. Use row 3 as column names
colnames(sup_tbl_raw) <- as.character(unlist(sup_tbl_raw[3, ]))

# 2b. Drop rows 1–4 (title, notes, header row, extra descriptor row)
sup_tbl <- sup_tbl_raw[-c(1:4), ]

# 3. Select and rename relevant columns --------------------
#   - "Volume term" -> Structure
#   - "rCMRGIc (µmol/100 g/min.)" -> rCMRGIc
sup_clean <- sup_tbl %>%
  dplyr::select(
    `Volume term`,
...
# 4. Join rCMRGIc into core_df based on exact Structure ----
core_with_rCMRGIc <- core_df %>%
  dplyr::left_join(
    sup_clean %>% dplyr::select(Structure, rCMRGIc),
    by = "Structure"
  )

# 5. (Optional) Confirm CI column names are intact ----------
# names(core_with_rCMRGIc)

# 6. Save result to a new CSV ------------------------------
write.csv(
  core_with_rCMRGIc,
  "core_df_out_with_rCMRGIc.csv",
  row.names = FALSE
)
