# --- Original cloud working directory (macOS / Dropbox) ---
# Project root: must contain R/, data/, and (auto-created) figs/ subfolders.
...
## Add global proportions (explicit)
global_props <- obs %>%
  filter(anatomy_group != "", anatomy_group != "Unmapped") %>%
  count(supercluster_term, name = "n_cells_global") %>%
  mutate(
    p_global = n_cells_global / sum(n_cells_global)
  )

celltype_table_long <- celltype_table_long %>%
  left_join(global_props, by = "supercluster_term") %>%
  mutate(
    enrichment_global = p_cells / p_global,
    log2_enrichment_global = log2(p_cells / p_global)
  )

celltype_proportion_table <- celltype_table_long %>%
  select(anatomy_group, supercluster_term, n_cells, p_cells, enrichment_global, log2_enrichment_global) %>%
  pivot_wider(
    names_from = supercluster_term,
    values_from = c(n_cells, p_cells, enrichment_global, log2_enrichment_global),
    values_fill = 0,
    names_sep = "_"
  ) %>%
  left_join(
...
exclude <- c("p_Bergmann glia",	#Not a good pan-region predictor. It is essentially cerebellum-specific.
             "p_Choroid plexus",	#Reflects sampling/proximity/contamination rather than regional composition.
             "p_Ependymal"	#Anatomically restricted and ventricle-associated.
)

# Detect all p_ columns and optionally exclude some (used for correlations + plots)
all_p <- names(analysis_df)[grepl("^p_", names(analysis_df))]
predictors <- setdiff(all_p, exclude)

setdiff(exclude, all_p)

cat("\nPredictors used for correlations and plots:\n")
print(predictors)
...
make_plot_subset <- function(df, predictors, label, file_slug) {
  if (nrow(df) < 3) {
    cat("  Skipping plot for ", label, ": only ", nrow(df), " regions.\n", sep = "")
    return(invisible(NULL))
  }
  plot_df <- df %>%
  ...
  # Region palette: try the project palette; fall back gracefully if a region
  # is not registered with the helpers in 0.01_plot_settings.R.
  region_scale <- tryCatch({
    check_region_palette(df, region_col = "anatomy_group")
    df_ordered <- set_region_order(df, region_col = "anatomy_group")
    plot_df$anatomy_group <- factor(plot_df$anatomy_group,
                                    levels = levels(df_ordered$anatomy_group))
    scale_color_regions()
  }, error = function(e) {
  ...
  p <- ggplot(plot_df, aes(x = prop, y = rcmr_value, color = anatomy_group)) +
    ...
    labs(
      ...
      subtitle = paste0("n = ", nrow(df), " regions"),
  ...
  ggsave(
    filename = paste0("figs/p_nonneuronal_", file_slug, ".pdf"),
    plot = p, width = 10, height = 7, units = "in"
  )
  ggsave(
    filename = paste0("figs/p_nonneuronal_", file_slug, ".jpg"),
...
write.csv(
  tel_df,
  "data/nonneuronal_celltype_proportions_by_region_telencephalon.csv",
  row.names = FALSE
)
write.csv(
  nontel_df,
  "data/nonneuronal_celltype_proportions_by_region_nontelencephalon.csv",
  row.names = FALSE
)
...
cor_tel    <- run_cor_subset(tel_df,    predictors, "Telencephalon")
cor_nontel <- run_cor_subset(nontel_df, predictors, "Non-telencephalon")

# Combined correlation table (long)
cor_combined <- dplyr::bind_rows(cor_tel, cor_nontel)
write.csv(
  cor_combined,
  "data/spearman_correlations_by_telencephalon.csv",
  row.names = FALSE
)

p_tel    <- make_plot_subset(tel_df,    predictors, "Telencephalon",     "telencephalon")
p_nontel <- make_plot_subset(nontel_df, predictors, "Non-telencephalon", "nontelencephalon")
...
plot_df_all <- analysis_df %>%
  dplyr::select(anatomy_group, rcmr_value, division, all_of(predictors)) %>%
  tidyr::pivot_longer(
    cols = all_of(predictors),
    names_to = "predictor",
    values_to = "prop"
...
region_scale_all <- tryCatch({
  ...
  analysis_df_ord <- set_region_order(analysis_df, region_col = "anatomy_group")
  plot_df_all$anatomy_group <- factor(plot_df_all$anatomy_group,
                                      levels = levels(analysis_df_ord$anatomy_group))
  scale_color_regions()
}, error = function(e) {
...
ggsave(
  filename = "figs/p_nonneuronal_by_telencephalon.pdf",
  plot = p_combined, width = 14, height = 9, units = "in"
)
ggsave(
  filename = "figs/p_nonneuronal_by_telencephalon.jpg",
...
ggsave(
  filename = "figs/p_nonneuronal_overlay_telencephalon.pdf",
  plot = p_overlay, width = 12, height = 8, units = "in"
)
ggsave(
  filename = "figs/p_nonneuronal_overlay_telencephalon.jpg",
  plot = p_overlay, width = 12, height = 8, units = "in", dpi = 300
)

cat("\nDone. Outputs:\n")
cat("  data/telencephalon_classification.csv\n")
cat("  data/nonneuronal_celltype_proportions_by_region_telencephalon.csv\n")
cat("  data/nonneuronal_celltype_proportions_by_region_nontelencephalon.csv\n")
cat("  data/spearman_correlations_by_telencephalon.csv\n")
cat("  figs/p_nonneuronal_telencephalon.{pdf,jpg}\n")
cat("  figs/p_nonneuronal_nontelencephalon.{pdf,jpg}\n")
cat("  figs/p_nonneuronal_by_telencephalon.{pdf,jpg}\n")
cat("  figs/p_nonneuronal_overlay_telencephalon.{pdf,jpg}\n")
