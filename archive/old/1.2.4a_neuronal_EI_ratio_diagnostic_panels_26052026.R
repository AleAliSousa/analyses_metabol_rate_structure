...
###################################################################
# Diagnostic 2 x 2 panels: E, I, E + I, and log2(E/I)
###################################################################
# These plots are intended to check whether the weak raw E:I result is
# driven by denominator instability, skewed ratios, or loss of information
# from collapsing excitatory and inhibitory proportions into a single ratio.

pseudocount_EI <- 1e-4

analysis_df_diagnostic <- analysis_df %>%
  mutate(
    p_cells_E_plus_I = p_cells_Excitatory_projection + p_cells_Inhibitory_interneuron,
    log2_EI_ratio_pseudocount = log2(
      (p_cells_Excitatory_projection + pseudocount_EI) /
        (p_cells_Inhibitory_interneuron + pseudocount_EI)
    )
  )

write.csv(
  analysis_df_diagnostic,
  "data/neuronal_EI_ratio_diagnostic_variables_by_region_with_rcmr_telencephalon_split.csv",
  row.names = FALSE
)

# Correlation table for all four diagnostic variables, separately by division
# and for all mapped regions.
diagnostic_variables <- c(
  "p_cells_Excitatory_projection",
  "p_cells_Inhibitory_interneuron",
  "p_cells_E_plus_I",
  "log2_EI_ratio_pseudocount"
)

diagnostic_correlations <- bind_rows(
  purrr::map_dfr(diagnostic_variables, ~safe_spearman(
    analysis_df_diagnostic %>% filter(is_telencephalon),
    "Telencephalon",
    variable = .x
  )),
  purrr::map_dfr(diagnostic_variables, ~safe_spearman(
...
print(diagnostic_correlations, n = Inf)

write.csv(
  diagnostic_correlations,
  "data/spearman_correlations_rcmr_neuronal_EI_diagnostic_variables_telencephalon_split.csv",
  row.names = FALSE
)

plot_df_diagnostic <- analysis_df_diagnostic %>%
  select(
    anatomy_group, division, is_telencephalon, rcmr_value,
    p_cells_Excitatory_projection,
    p_cells_Inhibitory_interneuron,
    p_cells_E_plus_I,
    log2_EI_ratio_pseudocount
  ...
  pivot_longer(
    cols = all_of(diagnostic_variables),
    names_to = "predictor",
  ...
  mutate(
    predictor_label = factor(
      predictor,
      levels = diagnostic_variables,
      labels = c(
        "Excitatory projection proportion",
        "Inhibitory interneuron proportion",
        "Excitatory + inhibitory proportion",
        paste0("log2((E + ", pseudocount_EI, ") / (I + ", pseudocount_EI, "))")
      )
    )
  ) %>%
  filter(is.finite(predictor_value), !is.na(rcmr_value), !is.na(division))

if (nrow(plot_df_diagnostic) < 4) {
  cat("\nSkipping diagnostic plots: insufficient finite data.\n")
} else {

  # Region-colored plot, with one row per diagnostic variable and one column per division.
  region_scale_diagnostic <- tryCatch({
    check_region_palette(plot_df_diagnostic, region_col = "anatomy_group")
    present_diag <- intersect(region_order, unique(as.character(plot_df_diagnostic$anatomy_group)))
    plot_df_diagnostic$anatomy_group <- factor(plot_df_diagnostic$anatomy_group, levels = present_diag)
    ggplot2::scale_color_manual(values = region_palette, drop = TRUE)
  }, error = function(e) {
    message("Region palette unavailable for diagnostic plot (", e$message,
            "); using default ggplot palette.")
    NULL
  })

  p_diagnostic_by_division <- ggplot(
    plot_df_diagnostic,
    aes(x = predictor_value, y = rcmr_value, color = anatomy_group)
  ) +
    ...
    facet_grid(predictor_label ~ division, scales = "free_x") +
    labs(
      title = "Diagnostic neuronal composition predictors vs rCMRGlc",
      subtitle = "Testing E, I, E + I, and log2 E:I separately by telencephalon status",
      x = "Predictor value",
      y = "rCMRGlc (µmol/100 g/min.)",
      color = "Region"
    ) +
  ...
  if (!is.null(region_scale_diagnostic)) p_diagnostic_by_division <- p_diagnostic_by_division + region_scale_diagnostic

  print(p_diagnostic_by_division)

  ggsave(
    filename = "figs/p_neuronal_EI_diagnostic_predictors_by_telencephalon_grid.pdf",
    plot = p_diagnostic_by_division, width = 12, height = 10, units = "in"
  )
  ggsave(
    filename = "figs/p_neuronal_EI_diagnostic_predictors_by_telencephalon_grid.jpg",
    plot = p_diagnostic_by_division, width = 12, height = 10, units = "in", dpi = 300
  )

  # Compact 2 x 2 version: one panel per diagnostic variable, separate lines by division.
  p_diagnostic_2x2 <- ggplot(
    plot_df_diagnostic,
    aes(x = predictor_value, y = rcmr_value, color = division, fill = division)
  ) +
    geom_point(size = 2.5, alpha = 0.85) +
    geom_smooth(method = "lm", se = TRUE, alpha = 0.15) +
    stat_poly_eq(
    ...
    labs(
      title = "Diagnostic neuronal composition predictors vs rCMRGlc",
      subtitle = "One panel per predictor; separate linear fits for telencephalon and non-telencephalon",
      x = "Predictor value",
      y = "rCMRGlc (µmol/100 g/min.)",
      color = "Division",
      fill = "Division"
  ...
  print(p_diagnostic_2x2)

  ggsave(
    filename = "figs/p_neuronal_EI_diagnostic_predictors_2x2_overlay.pdf",
    plot = p_diagnostic_2x2, width = 10, height = 8, units = "in"
  )
  ggsave(
    filename = "figs/p_neuronal_EI_diagnostic_predictors_2x2_overlay.jpg",
    plot = p_diagnostic_2x2, width = 10, height = 8, units = "in", dpi = 300
  )
}

cat("\nAdditional diagnostic outputs:\n")
cat("  data/neuronal_EI_ratio_diagnostic_variables_by_region_with_rcmr_telencephalon_split.csv\n")
cat("  data/spearman_correlations_rcmr_neuronal_EI_diagnostic_variables_telencephalon_split.csv\n")
cat("  figs/p_neuronal_EI_diagnostic_predictors_by_telencephalon_grid.{pdf,jpg}\n")
cat("  figs/p_neuronal_EI_diagnostic_predictors_2x2_overlay.{pdf,jpg}\n")
