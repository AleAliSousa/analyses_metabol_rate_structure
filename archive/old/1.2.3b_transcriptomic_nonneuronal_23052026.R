...
## 2) Add global proportions (explicit)
# ---- Global proportions per cell type ----
global_props <- obs %>%
  filter(anatomy_group != "", anatomy_group != "Unmapped") %>%
...
## 3) Join global proportions + compute enrichment
...
## 4) Then pivot (counts + proportions + enrichment)
celltype_proportion_table <- celltype_table_long %>%
...
write.csv(
  ...
  "data/nonneuronal_celltype_proportions_by_region.csv",
  row.names = FALSE
)
