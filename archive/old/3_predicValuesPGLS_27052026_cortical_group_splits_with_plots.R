...
# Output directories used throughout the script
...
## -------------------------
## PLOT I-IV helpers for both cortical grouping schemes
## -------------------------

plot_group_levels <- function(group_col) {
  if (identical(group_col, "CorticalGroup2")) return(cortical_2_group_levels)
  if (identical(group_col, "CorticalGroup3")) return(cortical_3_group_levels)
  NULL
}

prep_grouped_plot_df <- function(df, group_col) {
  df %>%
    mutate(
      GroupForPlot = factor(.data[[group_col]], levels = plot_group_levels(group_col))
    )
}

make_plot_prediction <- function(df, group_col, group_label) {
  plot_df <- prep_grouped_plot_df(df, group_col)

  ggplot(plot_df, aes(x = VarLabel)) +
    ...
    coord_flip() +
    facet_wrap(vars(GroupForPlot, Model), scales = "free_x") +
    theme_bw() +
    labs(
      title = paste0("Prediction: Brownian Motion vs Pagel's lambda by ", group_label),
      subtitle = "Part-whole corrected. Predictor = Rest of Brain (Total - Structure)",
...
make_plot_standardized_prediction <- function(df, group_col, group_label) {
  plot_df <- df %>%
    group_by(Variable) %>%
    mutate(
    ...
    ungroup() %>%
    prep_grouped_plot_df(group_col)
  ...
  ggplot(plot_df, aes(x = VarLabel)) +
    ...
    theme_bw() +
    labs(
      title = paste0("Standardized Prediction Error by ", group_label),
      subtitle = "0 = Min value for structure, 1 = Max value for structure across CI and observed. Predictor = Rest of Brain",
...
make_plot_human_mosaicism <- function(df, group_col, group_label) {
  plot_df <- prep_grouped_plot_df(df, group_col)

  ggplot(plot_df, aes(x = reorder(VarLabel, PropDiff), y = PropDiff, color = Direction)) +
    ...
    theme_bw() +
    labs(
      title = paste0("Human Brain Mosaicism by ", group_label),
      subtitle = "Part-whole corrected. Predictor = Rest of Brain (Total - Structure)",
...
make_plot_lambda_profile <- function(df_profile, df_mle, group_col, group_label) {
  profile_plot_df <- prep_grouped_plot_df(df_profile, group_col)
  mle_plot_df <- prep_grouped_plot_df(df_mle, group_col)

  ggplot(profile_plot_df, aes(x = lambda, y = LogLik)) +
    geom_line(color = "#377eb8", size = 1) +
    geom_point(data = mle_plot_df, aes(x = lambda, y = LogLik), color = "red", size = 3) +
    geom_vline(data = mle_plot_df, aes(xintercept = lambda), linetype = "dashed", color = "red", alpha = 0.5) +
    geom_text(data = mle_plot_df, aes(label = sprintf("%.2f", lambda), x = 0.1, y = LogLik),
              color = "red", size = 3, hjust = 0, vjust = 1) +
    facet_wrap(
      vars(GroupForPlot, Variable),
      scales = "free_y",
      labeller = labeller(Variable = function(x) label_region(x))
    ) +
    theme_bw() +
    labs(
      title = paste0("lambda Likelihood Profiles by ", group_label),
      subtitle = "Predictor = Rest of Brain. Curve shows model fit at fixed lambda values. Red dot = MLE.",
...
save_plot_pair <- function(plot_obj, stem, width, height) {
  ggsave(paste0("figs/", stem, ".png"), plot_obj, width = width, height = height, dpi = 300)
  ggsave(paste0("figs/", stem, ".pdf"), plot_obj, width = width, height = height)
}

## -------------------------
## PLOT I-II: prediction plots for both grouping schemes
## -------------------------

p1_cortical2 <- make_plot_prediction(core_df, "CorticalGroup2", "cortical vs non-cortical group")
p1_cortical3 <- make_plot_prediction(core_df, "CorticalGroup3", "cortical gray / white matter / non-cortical group")

p2_cortical2 <- make_plot_standardized_prediction(core_df, "CorticalGroup2", "cortical vs non-cortical group")
p2_cortical3 <- make_plot_standardized_prediction(core_df, "CorticalGroup3", "cortical gray / white matter / non-cortical group")

# Backward-compatible aliases: these preserve the previous object names as the 2-way grouping.
p1 <- p1_cortical2
p2 <- p2_cortical2
...
p3_cortical2 <- make_plot_human_mosaicism(final_df, "CorticalGroup2", "cortical vs non-cortical group")
p3_cortical3 <- make_plot_human_mosaicism(final_df, "CorticalGroup3", "cortical gray / white matter / non-cortical group")
...
## -------------------------
## PLOT IV: lambda likelihood profiles for both grouping schemes
## -------------------------
...
p4_cortical2 <- make_plot_lambda_profile(df_profile, df_mle, "CorticalGroup2", "cortical vs non-cortical group")
p4_cortical3 <- make_plot_lambda_profile(df_profile, df_mle, "CorticalGroup3", "cortical gray / white matter / non-cortical group")
...
# ============================================================
# Save PGLS summary plots for both grouping schemes
# ============================================================

dir.create("figs", showWarnings = FALSE, recursive = TRUE)

save_plot_pair(p1_cortical2, "plot_prediction_BM_vs_pagel_by_cortical2_group", width = 10, height = 7)
save_plot_pair(p1_cortical3, "plot_prediction_BM_vs_pagel_by_cortical3_group", width = 12, height = 8)

save_plot_pair(p2_cortical2, "plot_standardized_prediction_error_by_cortical2_group", width = 10, height = 7)
save_plot_pair(p2_cortical3, "plot_standardized_prediction_error_by_cortical3_group", width = 12, height = 8)

save_plot_pair(p3_cortical2, "plot_human_brain_mosaicism_by_cortical2_group", width = 12, height = 8)
save_plot_pair(p3_cortical3, "plot_human_brain_mosaicism_by_cortical3_group", width = 13, height = 9)

save_plot_pair(p4_cortical2, "plot_pagel_lambda_profile_by_cortical2_group", width = 12, height = 9)
save_plot_pair(p4_cortical3, "plot_pagel_lambda_profile_by_cortical3_group", width = 13, height = 10)

# Keep old filenames as aliases for the 2-way grouping, so downstream references do not break.
save_plot_pair(p1, "plot_prediction_BM_vs_pagel", width = 10, height = 7)
save_plot_pair(p2, "plot_standardized_prediction_error", width = 10, height = 7)
save_plot_pair(p3, "plot_human_brain_mosaicism", width = 12, height = 8)
save_plot_pair(p4, "plot_pagel_lambda_profile", width = 12, height = 9)
...
write_csv(
  core_with_rCMRGlc,
  "data/core_with_rCMRGlc_predicted_volumes.csv"
)
...
run_rCMRGlc_group_analyses <- function(core_with_rCMRGlc,
  ...
  write_csv(
    cor_pearson,
    paste0("tables/correlation_rCMRGlc_prediction_error_by_", output_suffix, "_pearson.csv")
  )
  write_csv(
    cor_spearman,
    paste0("tables/correlation_rCMRGlc_prediction_error_by_", output_suffix, "_spearman.csv")
  )
  ...
  ggsave(paste0("figs/plot_loess_errorbars_BM_rCMRGlc_by_", output_suffix, ".png"), p_loess_bars, width = 9, height = 5, dpi = 300)
  ggsave(paste0("figs/plot_polynomial_comparison_BM_rCMRGlc_by_", output_suffix, ".png"), p_poly_compare, width = 11, height = 5.5, dpi = 300)
  ggsave(paste0("figs/plot_quadratic_CI_BM_rCMRGlc_by_", output_suffix, ".png"), p_quad_ci, width = 9, height = 5, dpi = 300)

  ggsave(paste0("figs/plot_loess_errorbars_BM_rCMRGlc_by_", output_suffix, ".pdf"), p_loess_bars, width = 9, height = 5)
  ggsave(paste0("figs/plot_polynomial_comparison_BM_rCMRGlc_by_", output_suffix, ".pdf"), p_poly_compare, width = 11, height = 5.5)
  ggsave(paste0("figs/plot_quadratic_CI_BM_rCMRGlc_by_", output_suffix, ".pdf"), p_quad_ci, width = 9, height = 5)
